...
export default function MatchCard({ match }: MatchCardProps) {
  ...
  const { user, compatibilityScore, matchFactors, recommendedActivity, conversationStarters } = match;
    ...
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden animate-fade-in hover:shadow-xl transition-shadow">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 p-6 text-white">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            {user.age && (
              <p className="text-indigo-100">{user.age} years old</p>
            )}
          ...
          <div className="text-right">
            ...
            <div className="text-sm text-indigo-100">match</div>
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        {matchFactors.length > 0 && (
          <div>
            <h4 className="text-sm font-semibold text-gray-500 uppercase mb-2">Why you match</h4>
        ...
        <div className="space-y-3">
          {user.career && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Work</p>
          ...
          {user.interests && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Interests</p>
              <p className="text-gray-800 italic">"{user.interests.substring(0, 150)}{user.interests.length > 150 ? '...' : ''}"</p>
            </div>
          )}

          {user.keystoneValues && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Values</p>
              <p className="text-gray-800 italic">"{user.keystoneValues.substring(0, 150)}{user.keystoneValues.length > 150 ? '...' : ''}"</p>
            </div>
          )}
        </div>
        ...
        {expanded && (
          <div className="pt-6 border-t border-gray-200 space-y-4 animate-slide-up">
            {user.lifePhilosophy && (
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-1">Life Philosophy</p>
            ...
            {user.favoriteBooks && (
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-1">Favorite Books</p>
            ...
            {user.favoriteAuthors && (
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-1">Favorite Authors</p>
            ...
            {user.hobbies && (
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-1">Hobbies</p>
            ...
            {user.whatImLookingFor && (
              <div>
                ...
                <p className="text-gray-600 italic">"{user.whatImLookingFor}"</p>
              </div>
            )}
          </div>
        )}
